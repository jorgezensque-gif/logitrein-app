...
def _supa_url(): return st.secrets.get("SUPABASE_URL","")
def _supa_key(): return st.secrets.get("SUPABASE_KEY","")
...
def _supa_set(key, value):
    ...
    try:
        ...
        st.error(f"⚠️ Erro ao salvar no Supabase: {e}")
        return False
...
def init_banco():
    if 'banco_users' not in st.session_state:
        ...
        if supa_users and isinstance(supa_users, list) and len(supa_users) > 0:
            ...
            st.session_state.banco_supa_ok = True
        else:
            # Supabase vazio ou erro — usa padrão mas NÃO sobrescreve se já teve dados
            st.session_state.banco_users = json.loads(json.dumps(USUARIOS_PADRAO))
            st.session_state.banco_supa_ok = False
...
def save_users(u):
    ...
    # Persiste no Supabase de forma síncrona
