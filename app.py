...
def lt_file():
    candidates = ['logitrein.html','logitrein_v12_integrado.html','logitrein_v12.html',
                  'logitrein4.html','logitrein_4.html','LogiTrein.html']
    for f in candidates:
        if file_exists(f): return f
    # Busca qualquer html que comece com "logit" (case insensitive), exceto banco
    base = Path(__file__).parent
    for p in sorted(base.glob('*.html')):
        if p.name.lower().startswith('logit') and 'banco' not in p.name.lower():
            return p.name
    return None
...
# ── Supabase helpers (banco) ──────────────────────────────────
def _supa_url():
    v = st.secrets.get("SUPABASE_URL","")
    return ''.join(v.split())
def _supa_key():
    v = st.secrets.get("SUPABASE_KEY","")
    return ''.join(v.split())
def _supa_ok():  return _supa_url().startswith("http")

def _supa_get(key):
    if not _supa_ok(): return None
    try:
        import urllib.request, json as _json
        url = f"{_supa_url()}/rest/v1/lt_store?key=eq.{key}&select=value"
...
def _supa_set(key, value):
    if not _supa_ok(): return False
    import urllib.request, json as _json, urllib.error
    try:
        # Usa PATCH para atualizar registro existente
        data = _json.dumps({"value": value}).encode()
        url = f"{_supa_url()}/rest/v1/lt_store?key=eq.{key}"
        ...
        body = e.read().decode()
        st.error(f"❌ Supabase erro {e.code}: {body[:200]}")
        return False
    except Exception as e:
...
def init_banco():
    # Sempre tenta carregar do Supabase (fonte de verdade)
    if _supa_ok():
        supa_users = _supa_get("lt_banco_users")
        if supa_users and isinstance(supa_users, list) and len(supa_users) > 0:
            st.session_state.banco_users = supa_users
        elif 'banco_users' not in st.session_state:
...
with st.sidebar:
    ...
    if st.session_state.banco_user_id:
        ...
        if me:
            ...
            st.markdown("---")
            # Backup do banco
            users_json = json.dumps(get_users(), ensure_ascii=False, indent=2)
            st.download_button(
                label="💾 Exportar Backup",
                data=users_json,
                file_name=f"banco_backup_{datetime.now().strftime('%Y%m%d_%H%M')}.json",
                mime="application/json",
                use_container_width=True
            )
            bk = st.file_uploader("📥 Importar Backup", type=["json"], key="banco_bk")
            if bk:
                try:
                    restored = json.loads(bk.read())
...
if tela == 'portal':
    ...
    with c1:
        f_found = lt_file()
        if f_found:
            status_html = f"✅ <span style='color:#22c55e'>{f_found} ({get_size(f_found)})</span>"
        else:
            status_html = "❌ <span style='color:#e94b4b'>logitrein.html não encontrado no repositório</span>"
        st.markdown(f'<div class="card"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:1rem;color:#fff;margin-bottom:6px">🏭 Logitrein v12</div><div style="font-size:.81rem;color:#7a9bbf;line-height:1.5">Gestão operacional, equipes, ponto e DP integrado.</div><div style="font-size:.7rem;margin-top:8px">{status_html}</div></div>', unsafe_allow_html=True)
        if st.button("🚀 Abrir Logitrein v12", use_container_width=True, disabled=f_found is None):
            st.session_state.tela='logitrein'; st.rerun()
        if not f_found:
            up = st.file_uploader("📁 Fazer upload do logitrein.html", type=['html'], key='up_lt_portal')
            if up:
                (Path(__file__).parent / 'logitrein.html').write_bytes(up.read())
                st.success("✅ Arquivo salvo! Clique em Abrir Logitrein v12."); st.rerun()
    with c2:
...
# ══════════════════════════════════════════════
    ...
    # Busca logitrein_db.html (com Supabase) ou fallback para original
    for candidate in ['logitrein_db.html','logitrein_final.html','logitrein.html']:
        p = Path(__file__).parent / candidate
        if p.exists():
            html_content = p.read_text(encoding='utf-8')
            # Injeta credenciais Supabase — remove TODA whitespace/newline da chave
            supa_url = ''.join(st.secrets.get("SUPABASE_URL","").split())
            supa_key = ''.join(st.secrets.get("SUPABASE_KEY","").split())
            html_content = html_content.replace('%%SUPABASE_URL%%', supa_url)
            html_content = html_content.replace('%%SUPABASE_KEY%%', supa_key)
            break
    else:
        html_content = None
    ...
    if html_content:
        ...
        # ── Captura dados enviados pelo Logitrein via query_params ──
        lt_save = st.query_params.get("lt_save", None)
        if lt_save:
            try:
                import json as _json
                payload = _json.loads(lt_save)
                key = payload.get("key")
                value = payload.get("value")
                if key and value is not None:
                    ok = _supa_set(key, value)
                    if ok:
        ...
        # ── Injeta script que envia dados ao Python via query_params ──
        save_bridge = """
<script>
(function(){
  // Substitui supaSet para enviar ao Python via URL
  var _origSupaSet = window.supaSet || function(){};
  window.supaSet = async function(key, value){
    // Tenta o supaSet original primeiro (fetch direto)
    try{ await _origSupaSet(key, value); } catch(e){}
    // Também envia ao Python via query_params (garantia)
    try{
      var payload = JSON.stringify({key: key, value: value});
      var encoded = encodeURIComponent(payload);
      // Usa fetch para o próprio app Streamlit via postMessage
      window.parent.postMessage({type:'lt_save',key:key,value:value},'*');
    }catch(e){}
  };
  // Captura postMessage do iframe e redireciona para query_params
  window.addEventListener('message', function(e){
    if(e.data && e.data.type === 'lt_save'){
      var payload = JSON.stringify({key: e.data.key, value: e.data.value});
      var url = window.location.href.split('?')[0] + '?lt_save=' + encodeURIComponent(payload);
      // Salva sem recarregar usando history
      try{ window.history.replaceState({}, '', url); }catch(ex){}
    }
  });
})();
</script>"""
        # Injeta bridge antes do </body>
        html_with_bridge = html_content.replace('</body>', save_bridge + '</body>', 1)
        components.html(html_with_bridge, height=920, scrolling=True)
    else:
        st.error("Arquivo logitrein_db.html não encontrado.")
# ══════════════════════════════════════════════
...
# ══════════════════════════════════════════════
    ...
    # ── Supabase status ──
    if not _supa_ok():
        st.error("❌ Supabase não configurado. Verifique os Secrets.")
    ...
    # ── INVESTIMENTOS ──
        ...
        st.markdown("##### 💰 Posições — Clique para Resgatar")
        c1,c2 = st.columns(2)
        with c1:
            poup = me.get('poupanca',0)
            st.markdown(f'<div class="metric-box"><div class="metric-label">Poupança</div><div class="metric-value" style="color:#00c9a7">{fmt(poup)}</div></div>',unsafe_allow_html=True)
            if poup > 0 and st.button(f"💸 Resgatar Poupança ({fmt(poup)})", key="resg_poup"):
                update_saldo(me['id'], poup)
                add_tx(me['id'],'investimento','Resgate Poupança',poup,'in')
                users=get_users()
                for u in users:
                    if u['id']==me['id']: u['poupanca']=0
                save_users(users); st.success(f"✅ {fmt(poup)} resgatado!"); st.rerun()
        with c2:
            inv = me.get('investimentos',0)
            st.markdown(f'<div class="metric-box"><div class="metric-label">CDB/Tesouro/LCI</div><div class="metric-value" style="color:#93b4f7">{fmt(inv)}</div></div>',unsafe_allow_html=True)
            if inv > 0 and st.button(f"💸 Resgatar Investimentos ({fmt(inv)})", key="resg_inv"):
                update_saldo(me['id'], inv)
                add_tx(me['id'],'investimento','Resgate CDB/Tesouro/LCI',inv,'in')
                users=get_users()
                for u in users:
                    if u['id']==me['id']: u['investimentos']=0
                save_users(users); st.success(f"✅ {fmt(inv)} resgatado!"); st.rerun()
