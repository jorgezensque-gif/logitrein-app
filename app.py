...
def init_banco():
    if 'banco_users' not in st.session_state:
        ...
        if supa_users and isinstance(supa_users, list) and len(supa_users) > 0:
            ...
            # Salva padrão no Supabase se vazio
            _supa_set("lt_banco_users", st.session_state.banco_users)
...
def save_users(u):
    ...
    # Persiste no Supabase de forma assíncrona via thread
    import threading
    threading.Thread(target=_supa_set, args=("lt_banco_users", u), daemon=True).start()
def get_me():
