...
# ══════════════════════════════════════════════
    ...
    if f:
        ...
        # srcdoc: injeta HTML diretamente no iframe — sem data: URI, sem CSP externa, sem limite de tamanho
        # Escapa aspas duplas para não quebrar o atributo srcdoc
        srcdoc = html_content.replace('&', '&amp;').replace('"', '&quot;')
        iframe_html = f'''<style>
body{{margin:0;padding:0;background:#09182e;}}
...
<iframe id="ltf"
  ...
  allowfullscreen>
        ...
        components.html(iframe_html, height=870, scrolling=False)
