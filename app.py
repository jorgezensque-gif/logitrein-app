...
# ══════════════════════════════════════════════
    ...
    if f:
        ...
        # Bridge: injeta listener postMessage no HTML para capturar folha fechada
        bridge_script = """
<script>
(function(){
  // Escuta mensagens do Streamlit pai
  window.addEventListener('message', function(e){
    if(e.data && e.data.type === 'lt_get_folha_pendente'){
      // Streamlit perguntando se há folha nova — não usado aqui
    }
  });
})();
</script>
"""
        html_with_bridge = html_content.replace('</body>', bridge_script + '</body>')

        # Componente com callback JS→Python via query param trick
        receiver_js = """
<script>
window.addEventListener('message', function(e){
  if(e.data && e.data.type === 'lt_folha_fechada'){
    // Salva no sessionStorage do frame pai via postMessage
    try {
...
"""
        # Usa st.components.v1.html com postMessage capture via query_params
        # A bridge real é feita via st_javascript se disponível, senão mostra aviso
        import streamlit.components.v1 as components

        # Captura via URL param (workaround robusto para Streamlit)
        folha_json = st.query_params.get("lt_folha", None)
        if folha_json:
            try:
                folha_data = json.loads(folha_json)
                if folha_data.get("type") == "lt_folha_fechada":
                    users = get_users()
                    depositados, sem_conta = [], []
                    mes_ano = folha_data.get("mesAno", "")
                    for pag in folha_data.get("pagamentos", []):
                        nome = (pag.get("nome") or "").lower().strip()
                        valor = float(pag.get("valor") or 0)
                        if valor <= 0:
                            continue
                        conta = next((u for u in users
                                      if u["role"] == "salario" and u["ativo"]
                                      and u["nome"].lower().strip() == nome), None)
                        if not conta:
                            conta = next((u for u in users
                                          if u["role"] == "salario" and u["ativo"]
                                          and nome.split()[0] in u["nome"].lower()), None)
                        if conta:
                            conta["saldo"] = float(conta.get("saldo", 0)) + valor
                            conta.setdefault("transacoes", []).insert(0, {
                                "tipo": "deposito",
                                "desc": f"💼 Salário {mes_ano} — Logitrein",
                                "valor": valor, "dir": "in", "ts": ts()
                            })
                            depositados.append(conta["nome"])
                        else:
                            sem_conta.append(pag.get("nome"))
                    save_users(users)
                    st.query_params.clear()
                    if depositados:
                        st.success(f"✅ Banco atualizado! Salários depositados: {', '.join(depositados)}")
                    if sem_conta:
                        st.warning(f"⚠️ Sem conta salário para: {', '.join(sem_conta)}. Crie no Banco → Usuários.")
            except Exception as ex:
                st.query_params.clear()
                st.error(f"Erro ao processar folha: {ex}")

        # Injeta script que envia postMessage convertendo para URL param
        postmsg_capture = """
<script>
(function(){
  window.addEventListener('message', function(e){
    if(e.data && e.data.type === 'lt_folha_fechada'){
      var encoded = encodeURIComponent(JSON.stringify(e.data));
      var url = window.parent.location.href.split('?')[0] + '?lt_folha=' + encoded;
...
"""
        final_html = html_with_bridge.replace('</body>', postmsg_capture + '</body>')
        components.html(final_html, height=950, scrolling=True)

        st.markdown("""
<div style='font-size:.72rem;color:#2a4060;margin-top:6px;padding:8px 12px;background:rgba(0,201,167,.05);border-radius:8px;border:1px solid rgba(0,201,167,.1)'>
🔗 <strong style='color:#00c9a7'>Integração ativa:</strong> Ao fechar a folha no Logitrein, os salários serão depositados automaticamente no Banco.
</div>""", unsafe_allow_html=True)
    else:
        st.error("Arquivo logitrein.html não encontrado. Faça upload no Painel Admin.")
