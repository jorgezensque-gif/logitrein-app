...
def init_banco():
    if 'banco_users' not in st.session_state:
        ...
        if supa_users and isinstance(supa_users, list) and len(supa_users) > 0:
            ...
            # Só usa padrão se Supabase realmente vazio (primeira vez)
...
# ══════════════════════════════════════════════
    ...
    # ── Supabase status ──
    if _supa_ok():
        test = _supa_get("lt4_sys_ver")
        if test is None:
            st.warning("⚠️ Supabase conectado mas sem resposta. Dados não serão persistidos.")
        # else silently ok
    else:
