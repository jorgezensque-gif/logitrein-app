...
def init_banco():
    ...
    if _supa_ok():
        try:
            ...
            # Garante que é lista de dicts com campos obrigatórios
            if isinstance(supa_users, list) and len(supa_users) > 0:
                valid = [u for u in supa_users if isinstance(u, dict) and 'login' in u and 'senha' in u]
                if valid:
                    st.session_state.banco_users = valid
                    loaded = True
        except Exception:
            pass
    if not loaded:
...
def get_me():
    uid = st.session_state.get('banco_user_id')
    ...
    if not me:
        ...
        with col:
            ...
            if st.button("🔑 Entrar no Sistema", use_container_width=True, type="primary"):
                users = get_users()
                try:
                    user = next((u for u in users if isinstance(u, dict) and u.get('login','').lower()==login_input.lower().strip() and u.get('senha','')==senha_input and u.get('ativo')), None)
                except Exception:
                    user = None
                if user:
