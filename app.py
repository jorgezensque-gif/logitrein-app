...
# ══════════════════════════════════════════════
    ...
    if f:
        ...
        import base64
        # Codifica o HTML em base64 e abre em data URI num iframe sem restrições de tamanho
        b64 = base64.b64encode(html_content.encode('utf-8')).decode('utf-8')
        data_uri = f"data:text/html;charset=utf-8;base64,{b64}"

        # Renderiza via iframe apontando para data URI — sem limite de tamanho, sem CSP externa
        iframe_html = f"""
<style>
  #lt-frame-wrap {{position:relative;width:100%;height:90vh;border-radius:12px;overflow:hidden;border:1px solid rgba(255,255,255,.07);}}
  #lt-frame {{width:100%;height:100%;border:none;display:block;}}
</style>
<div id="lt-frame-wrap">
  <iframe id="lt-frame" src="{data_uri}" allowfullscreen
...
"""
        ...
        components.html(iframe_html, height=int(0.92 * 900), scrolling=False)

        st.markdown("""<div style='font-size:.72rem;color:#2a4060;margin-top:4px;padding:6px 12px;
background:rgba(0,201,167,.05);border-radius:8px;border:1px solid rgba(0,201,167,.1)'>
🔗 <strong style='color:#00c9a7'>Integração ativa:</strong> Ao fechar a folha, salários são depositados no Banco automaticamente.
