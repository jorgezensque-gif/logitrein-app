...
# ══════════════════════════════════════════════
    ...
    # O logitrein.html tem 308KB — acima do limite do Streamlit components.html (~200KB)
    # Solução: GitHub Pages serve o arquivo diretamente; Streamlit embeda via iframe URL
    GHPAGES_URL = "https://jorgezensque-gif.github.io/logitrein-app/logitrein.html"

    import streamlit.components.v1 as components

    # Tenta carregar via GitHub Pages (URL pública, sem limite de tamanho)
    iframe_html = f'''<style>
body{{margin:0;padding:0;}}
iframe#ltf{{width:100%;height:96vh;border:none;display:block;}}
</style>
<iframe id="ltf" src="{GHPAGES_URL}" allowfullscreen
  allow="clipboard-read; clipboard-write; storage-access">
</iframe>'''
    components.html(iframe_html, height=880, scrolling=False)

    st.markdown(f"""
<div style='font-size:.72rem;color:#3a5070;margin-top:4px;padding:8px 14px;
background:rgba(255,255,255,.03);border-radius:8px;border:1px solid rgba(255,255,255,.06);
display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px'>
  <span>🌐 Servido via GitHub Pages · <code style='color:#00c9a7'>{GHPAGES_URL}</code></span>
  <a href='{GHPAGES_URL}' target='_blank'
     style='color:#00c9a7;font-weight:600;text-decoration:none;font-size:.72rem'>
    ↗ Abrir em nova aba
  </a>
