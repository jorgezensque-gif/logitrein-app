...
def _supa_set(key, value):
    ...
    try:
        data = _json.dumps({"key": key, "value": value}).encode()
        req = urllib.request.Request(
            f"{_supa_url()}/rest/v1/lt_store", data=data,
            headers={
                "apikey": _supa_key(),
...
def init_banco():
    if 'banco_users' not in st.session_state:
        ...
        if supa_users is None:
            st.info("ℹ️ Supabase retornou None para lt_banco_users")
            st.session_state.banco_users = json.loads(json.dumps(USUARIOS_PADRAO))
        elif not isinstance(supa_users, list):
            st.info(f"ℹ️ Supabase retornou tipo inesperado: {type(supa_users)}")
            st.session_state.banco_users = json.loads(json.dumps(USUARIOS_PADRAO))
        elif len(supa_users) == 0:
            st.info("ℹ️ Supabase retornou lista vazia — usando padrão")
            st.session_state.banco_users = json.loads(json.dumps(USUARIOS_PADRAO))
        else:
            st.success(f"✅ Carregado do Supabase: {len(supa_users)} usuários")
...
# ══════════════════════════════════════════════
    ...
    for candidate in ['logitrein_db.html','logitrein_final.html','logitrein.html']:
        ...
        if p.exists():
            ...
            # Injeta credenciais Supabase (definidas nos Secrets do Streamlit)
